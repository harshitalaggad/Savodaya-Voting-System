...
<body class="bg-img">
  ...
  <%
	...
	String CandidateSno = request.getParameter("candidateSno");
	
	int Sno = Integer.parseInt(CandidateSno);
   	//out.println(Sno);
	Candidate candidate = adminDao.getcandidatebySno(Sno);
	//out.print(candidate);
...
<div class="body-content">
  <div class="module">
    <h1 class ="h1">Edit Candidate</h1>
    <br>
    <form class="form" action="updatecandidate" method="post" enctype="multipart/form-data" autocomplete="off">
   ...
   <input type=hidden name="SNo" value="<%=candidate.getSNo()%>"><br>
		
		
		<label for="firstname">First Name:</label> 
		
		<input type="text" name="firstname" id="firstname" value="<%=candidate.getFirstName()%>"><br>
		<br> <br> 
		
		<label for="lastname">Last Name:</label> 
		
		<input type="text" name="lastname" id="lastname" value="<%=candidate.getLastName()%>"><br> <br> <br>
		
			<!-- Start -->
		<label> Year:</label> <label>II</label> 
		
		<input type="radio" <% if(candidate.getYear().equals("II")) {%>checked="checked" <%} %> name="year" value="II"> 
		
		<label>III</label> 
		
		<input type="radio" <% if(candidate.getYear().equals("III")) {%>checked="checked" <%} %> name="year" value="<%=candidate.getYear()%>"><br>
		
		<!-- End -->
		<br> <br> <div class="avatar"><label>Upload Image</label> <input type="file"
			name="image" value="<%=candidate.getImage()%>"> <br> <br>


		
		
			<input class="btn btn-block btn-primary" type="submit" class="para2" value="UPDATE">
		  </form>
		  
  </div>
  <a href="deletecandidate?Sno=<%=candidate.getSNo()%>" >
			<input class="btn btn-block btn-primary" type="submit"  class="para7"  value="DELETE"></a>
